...
% 如果「送审」，则「启用」下面这行命令
% 2026 年 4 月 27 日更新：
% 根据 2026 年新规范，送审版本不再完全不显示页眉
% 而是「保留页眉横线 + 右侧论文标题」
% 仅隐藏左侧「广西大学某士学位论文」与封面「广西大学图标」
% \gxublindtrue
% ————————————————————————————————————————————
% 若需「完全不显示页眉」（旧版送审表现），可启用下面这行
% 页眉横线、左右文字将均不显示
% 此命令与 \gxublindtrue 互不冲突，可独立或同时启用
% \gxunoheadertrue
% ————————————————————————————————————————————
% 「留空/注释掉」则对应栏目完全「不显示」
...
\renewcommand{\cntitle}{集成 QKD 器材\coverbreak 的安全性分析评估方法及\\ 宽谱滤波器研究} % 中文标题
% 如果中文标题中有「字母」的话，「书脊」的表现「可能不好」
% \gxuspinedoublecoltrue % 书脊默认按「单列」排版，取消注释则按「双列」排版
% \gxuspinefalse % 默认显示书脊，取消注释则不显示书脊
% 2026 年 4 月 27 日更新：
% 书脊单列模式下默认「字母不旋转」
% 「取消注释」则将字母旋转 90°，但效果很差
% \gxuspinerotatelettertrue
